...
class RomanianSalary extends BasicView {
	public function getValues($longBase) {
		$aReturn['cas'] = $this->setHealthFundTax($_REQUEST['ym']
			, $longBase);
		$aReturn['sanatate'] = $this->setHealthTax($_REQUEST['ym']
			, $longBase);
	...
	public function getOvertimes() {
		...
		$aReturn['os175'] = round($_REQUEST['os175'] * 1.75 * $_REQUEST['sn']
			/ $m, 0);
		$aReturn['os200'] = round($_REQUEST['os200'] * 2 * $_REQUEST['sn']
			/ $m, 0);
		return $aReturn;
	}
	/**
    ...
    **/
	...
	public function setWorkingDaysInMonth($lngDate, $include_easter) {
		$days_in_given_month = round((mktime(0, 0, 0, date('m', $lngDate)+1, 1
			, date('Y', $lngDate)) - mktime(0, 0, 0, date('m', $lngDate)
			, 1, date('Y', $lngDate))) / (60*60*24),0);
		$tmp_value = 0;
		for ($counter = 1; $counter <= $days_in_given_month; $counter++) {
			$current_day = mktime(0, 0, 0, date('m', $lngDate)
				, $counter, date('Y', $lngDate));
			if ((!in_array($current_day, $this->setHolidays($lngDate
					, $include_easter)))
				&& (strftime('%w', $current_day) != 0)
				&& (strftime('%w', $current_day) != 6)) {
					$tmp_value += 1;
				}
		}
		return $tmp_value;
